...
import { X, ChevronLeft, Zap, Star, Eye, Search, Award, Check, CreditCard, Lock } from "lucide-react";
...
export default function BoostProfileModal({ isOpen, onClose }: BoostProfileModalProps) {
  ...
  const { data: packagesData, isLoading: isLoadingPackages } = useGetBoostPackagesQuery();
    ...
    <div className="space-y-4 text-sm mt-6">
      <div className="flex gap-3">
        ...
        <div>
          ...
          <p className="text-[#8B97B5] text-xs">Top position in directory listings</p>
        </div>
      </div>
      <div className="flex gap-3">
        ...
        <div>
          ...
          <p className="text-[#8B97B5] text-xs">Increased profile views and engagement</p>
        </div>
      </div>
      <div className="flex gap-3">
        ...
        <div>
          ...
          <p className="text-[#8B97B5] text-xs">Appear first in search results</p>
        </div>
      </div>
      <div className="flex gap-3">
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#1E2554]">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[#8B97B5] text-sm">Feature your profile to increase visibility and engagement</p>
            </div>
          </div>
          <button 
            onClick={onClose}
            ...
            {step === 1 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="bg-[#11163C] border border-[#1E2554] rounded-xl p-4 flex items-center gap-4">
                  <h4 className="absolute text-xs font-bold text-[#8B97B5] -mt-12">Boosting Profile For:</h4>
                  <img 
                    src={profile?.profile_image || "https://images.unsplash.com/photo-1522778119026-d647f0596c20?w=500"} 
                    alt="Player Profile" 
                    className="w-12 h-12 rounded-full object-cover border-2 border-[#00E5FF]"
                  />
                  <div>
                    <h3 className="font-bold text-white text-lg">{profile?.full_name || "John Doe"}</h3>
                    <p className="text-sm text-[#00E5FF] capitalize">{profile?.designation || "Forward"}</p>
                  </div>
                </div>

                {/* Step 1: Packages */}
                <div>
                  <h3 className="text-white font-bold mb-4">Step 1: Choose Boost Duration</h3>
                  {isLoadingPackages ? (
                      ...
                      {packages.map((pkg: any) => (
                        ...
                        >
                          ...
                          <div className="flex justify-between items-center">
                            <span className="font-bold text-white text-lg">{pkg.duration}</span>
                            <span className="font-black text-[#00E5FF] text-2xl">€{pkg.finalPrice}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {/* Step 2: Date Range */}
                <div>
                  <h3 className="text-white font-bold mb-4">Step 2: Set Date Range</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-[#8B97B5] mb-2 uppercase tracking-wide">Start Date</label>
                      <input 
                        type="date" 
                        value={startDate}
                    ...
                    <div>
                      <label className="block text-xs font-bold text-[#8B97B5] mb-2 uppercase tracking-wide">End Date</label>
                      <input 
                        type="date" 
                        value={endDate}
            ...
            {step === 2 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
                <div className="flex items-center gap-2 mb-6">
                   ...
                   <h3 className="text-white font-bold text-xl">Payment Method</h3>
                </div>

                <form id="payment-form" onSubmit={handleSubmit(onPaymentSubmit)} className="space-y-8">
                   
                   {/* Card Information */}
                   <div className="bg-[#11163C] p-5 rounded-xl border border-[#1E2554] space-y-4">
                      <h4 className="text-sm font-bold text-white mb-2">Card Information</h4>
                      
                      <div>
                        <label className="block text-xs font-bold text-[#8B97B5] mb-1.5 uppercase">Card Number *</label>
                        <div className="relative">
                          ...
                          />
                          <CreditCard size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500" />
                        </div>
                      </div>

                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-bold text-[#8B97B5] mb-1.5 uppercase">Expiry Date *</label>
                           <input 
                              disabled
                              placeholder="MM/YY" 
                        ...
                        <div>
                          <label className="block text-xs font-bold text-[#8B97B5] mb-1.5 uppercase">CVV / CVC *</label>
                           <input 
                              disabled
                              placeholder="123" 
                              className="w-full bg-[#080B1A] border border-[#1E2550] text-gray-500 p-3.5 rounded-lg cursor-not-allowed"
                   ...
                   {/* Billing Address */}
                   <div className="bg-[#11163C] p-5 rounded-xl border border-[#1E2554] space-y-4">
                       <h4 className="text-sm font-bold text-white mb-2">Billing Address</h4>
                       
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                         <div className="md:col-span-2">
                            <label className="block text-xs font-bold text-[#8B97B5] mb-1.5 uppercase">Country / Region *</label>
                            <input 
                         ...
                         <div>
                            <label className="block text-xs font-bold text-[#8B97B5] mb-1.5 uppercase">City *</label>
                            <input 
                         ...
                         <div>
                            <label className="block text-xs font-bold text-[#8B97B5] mb-1.5 uppercase">Postal Code *</label>
                            <input 
                   ...
                   <div className="flex items-start gap-3 mt-4">
                     <input type="checkbox" id="terms" {...register("terms", { required: true })} className="mt-1" />
                     <label htmlFor="terms" className="text-sm text-[#8B97B5]">
                       I agree to the <span className="text-[#00E5FF] underline cursor-pointer">Terms of Service</span> and <span className="text-[#00E5FF] underline cursor-pointer">Privacy Policy</span>
                     </label>
                   </div>
                   
                   <div className="flex items-center gap-2 text-xs text-[#04B5A3]">
                     <Lock size={14} /> Secure Payment &nbsp;|&nbsp; 256-bit SSL Encrypted
                   </div>
                </form>
              </div>
            )}
            ...
            {step === 1 && (
              <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
                <div className="bg-[#11163C] border border-[#1E2554] rounded-2xl p-6 text-center shadow-lg relative overflow-hidden">
                   ...
                   <h3 className="text-xl font-bold text-white mb-6">Boost Summary</h3>
                   ...
                   <div className="space-y-4 text-sm mb-6">
                     ...
                     <div className="flex justify-between items-center pt-2">
                       ...
                       <span className="font-black text-2xl text-[#00E5FF]">€{selectedPackage?.finalPrice || "0"}</span>
                     </div>
                   </div>
                   
                   <div className="bg-[#080B1A] rounded-xl p-4 text-left border border-[#1E2550]">
                      <h4 className="text-xs font-bold text-[#8B97B5] mb-2 uppercase flex items-center gap-2"><Zap size={12}/> Boost Benefits</h4>
                      {renderBenefits()}
                   </div>
                </div>
                ...
                <div className="bg-[#11163C] border border-[#1E2554] rounded-2xl p-6 shadow-lg relative overflow-hidden">
                   ...
                   <div className="flex items-center gap-4 mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-[#00E5FF]">{selectedPackage?.duration}</p>
                      </div>
                   </div>
                   ...
                   <div className="space-y-4 text-sm border-t border-b border-[#1E2554] py-4 my-4">
                     <div className="flex justify-between items-center text-[#8B97B5]">
                       <span>Duration</span>
                       <span className="font-medium text-white">{selectedPackage?.duration}</span>
                     </div>
                     <div className="flex justify-between items-center text-[#8B97B5]">
                       <span>Start Date</span>
                       <span className="font-medium text-white">{startDate ? new Date(startDate).toLocaleDateString() : '-'}</span>
                     </div>
                     <div className="flex justify-between items-center text-[#8B97B5]">
                       <span>End Date</span>
                       <span className="font-medium text-white">{endDate ? new Date(endDate).toLocaleDateString() : '-'}</span>
                     </div>
                   </div>
                   ...
                   <div className="space-y-3 text-sm mb-6">
                     ...
                     <div className="flex justify-between items-center text-[#8B97B5]">
                       <span>Discount</span>
                       <span className="text-[#00E564]">-€{(selectedPackage?.price - selectedPackage?.finalPrice)?.toFixed(2) || 0}</span>
                     </div>
                   </div>

                   <div className="flex justify-between items-center bg-[#080B1A] p-4 rounded-xl border border-[#1E2550]">
                      <span className="font-bold text-white">Total Amount</span>
                      <span className="font-black text-2xl text-[#00E5FF]">€{selectedPackage?.finalPrice}</span>
                   </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  >
                    {isProcessing ? "Processing..." : (
                      <>
                        <Lock size={18} /> Complete Payment - €{selectedPackage?.finalPrice}
                      </>
                    )}
                  </button>
                  <button 
