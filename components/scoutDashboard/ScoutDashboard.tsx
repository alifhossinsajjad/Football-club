...
const StatCard: React.FC<Stat> = ({ label, value, subValue, icon, subColor }) => (
  <div className="bg-[#0f172a] border border-slate-800 rounded-2xl p-4 flex flex-col gap-2">
    <div className="flex justify-between items-start">
      <div className="p-2 bg-slate-800/50 rounded-lg text-cyan-400">
        {icon}
      </div>
...
const PlayerCard: React.FC<Player> = ({ name, role, nationality, age, image, isTopProspect }) => (
  <div className="relative min-w-[240px] bg-[#0f172a] border border-slate-800 rounded-3xl overflow-hidden group">
    ...
    <div className="p-5 -mt-12 relative z-10">
      ...
      <p className="text-slate-400 text-sm">{role}, {nationality}, {age}</p>
...
const EventItem: React.FC<ScoutingEvent> = ({ title, date, time, location }) => (
  <div className="flex items-center gap-4 p-4 bg-[#0f172a] border border-slate-800 rounded-2xl group cursor-pointer hover:border-slate-700 transition-colors">
    <div className="flex flex-col items-center justify-center w-14 h-14 bg-slate-800/50 rounded-xl text-center">
      <span className="text-[10px] uppercase font-bold text-purple-400">{date.month}</span>
      <span className="text-xl font-bold text-white leading-tight">{date.day}</span>
    </div>
...
const ScoutDashboard: React.FC = () => {
  const stats: Stat[] = [
    { label: 'Players Viewed', value: '342', subValue: '+48 wk', icon: <Eye size={20} />, subColor: 'text-cyan-400' },
    { label: 'Shortlisted', value: '28', subValue: '12 Active', icon: <Star size={20} />, subColor: 'text-purple-400' },
    { label: 'Events', value: '6', subValue: 'Sep 15', icon: <Calendar size={20} />, subColor: 'text-orange-400' },
    { label: 'Messages', value: '15', subValue: '5 Unread', icon: <MessageSquare size={20} />, subColor: 'text-emerald-400' },
  ];
    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans pb-24">
      ...
      <header className="px-6 pt-8 pb-6 flex justify-between items-center">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full border-2 border-emerald-500 p-0.5 overflow-hidden">
            <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Mike" alt="Profile" className="w-full h-full rounded-full object-cover" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-white">
              Welcome Back, <span className="text-purple-500 font-extrabold italic">Mike!</span>
            </h1>
            <p className="text-slate-500 text-xs">Head Scout • Manchester United</p>
          </div>
        </div>
      ...
      <section className="mb-10">
        <div className="px-6 flex justify-between items-end mb-6">
          <h2 className="text-xl font-bold text-white tracking-tight">Shortlisted Players</h2>
      ...
      <section className="px-6 mb-10">
        <h2 className="text-xl font-bold text-white tracking-tight mb-6">Upcoming Scouting Events</h2>
