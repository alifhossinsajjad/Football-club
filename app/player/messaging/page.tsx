                  ...
                  >
                    <div className="relative shrink-0">
                      <Avatar className="h-12 w-12 ring-1 ring-[#00E5FF]/20">
                        <AvatarImage src={(conv.avatar || conv.user?.avatar) || undefined} alt={conv.name || conv.user?.name || "U"} />
              ...
              <>
                <div className="sticky top-0 z-10 p-4 border-b border-[#2A3560]/70 flex items-center gap-3 bg-[#1A2049]/95 backdrop-blur-md">
                  <Avatar className="h-10 w-10 ring-1 ring-[#00E5FF]/30">
                    ...
                    <AvatarFallback className="bg-[#9C27B0]/50">{(selectedConv.name || selectedConv.user?.name || "U")[0]}</AvatarFallback>
                  </Avatar>
                  <div>
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-6 scrollbar-thin scrollbar-thumb-[#9C27B0]/60 scrollbar-track-[#1A2049]">
                  {loadingMessages ? (
                    ...
                    messages.map((msg) => {
                      const isOwn = msg.is_sender || msg.isOwn || msg.is_own;
                      return (
                        <div key={msg.id} className={cn("flex", isOwn ? "justify-end" : "justify-start")}>
                          ...
                          >
                            ...
                            <p className="text-xs mt-2 opacity-80 text-right">{msg.time || msg.created_at}</p>
                          </div>
                        </div>
                      );
                    })
