...
export default function ScoutAchievements({
  playerProfileData,
  isEditing,
      ...
      <div className="space-y-3">
        {editableAchievements.map((ach, index) => (
          <div key={index} className="flex items-start gap-3 group">
            {/* Only show Award icon in view mode */}
            {!isEditing && (
              <Award
                className="w-5 h-5"
                style={{ color: theme.colors.primaryCyan }}
            ...
            {isEditing ? (
              <div className="flex-1 flex items-center gap-2">
                ...
                <button
                  ...
                  className="text-red-500 bg-[#FF00001A] hover:text-red-400 p-3 rounded-lg transition"
                  title="Remove"
                >
